...
class AlignCollate(object):
    ...
    def __call__(self, batch):
        ...
        print(fr'recogniton AlignCollate self.imgH : {current_path} {self.imgH}')
        print(fr'recogniton AlignCollate self.imgW : {current_path} {self.imgW}')
        ...
        print(fr'recogniton AlignCollate resized_max_w : {current_path} {resized_max_w}')
...
def recognizer_predict(model, converter, test_loader, batch_max_length, \
    ...
    print(f'{current_path} recognizer_predict model : {model}')
    ...
    with torch.no_grad():
        for image_tensors in test_loader:
            ...
            # from PIL import Image
            #
            # # image_tensors는 텐서입니다.
            # # image_path는 이미지를 저장할 경로입니다.
            # def save_image(image_tensors, image_path):
            #     # 이미지 텐서를 PIL 이미지로 변환합니다.
            #     image_pil = TF.to_pil_image(image_tensors)
            #
            #     # PIL 이미지를 JPEG 파일로 저장합니다.
            #     image_pil.save(image_path, "JPEG")
            #
            # # 이미지 텐서를 저장할 경로를 지정합니다.
            # save_path = "C:/Users/TAMSystech/yjh/ipynb/deep-text-recognition-benchmark/data/th/test/0319/train/img/test_image.jpg"
            #
            # # 이미지 텐서를 저장합니다.
            # save_image(image, save_path)
            #
            # print(fr'이미지 저장 완료 텐서이미지 save_path : {save_path}')
    ...
    print(fr'recognizer_predict result : {current_path} {result} ')
...
def get_recognizer(recog_network, network_params, character, \
    ...
    print(fr'recognition.py get_recognizer character : {character}')

    converter = CTCLabelConverter(character, separator_list, dict_list)
    num_class = len(converter.character)

    print(fr'get_recognizer recog_network : {current_path} {recog_network} ')
    print(fr'get_recognizer num_class : {current_path} {num_class} ')
    ...
    if recog_network == 'generation1':
        ...
        print(fr'get_recognizer recog_network else : {current_path} {recog_network} ')

        recog_network = "easyocr.model.None_VGG_BiLSTM_CTC_th"
        #recog_network = "pythonlib_easyocr.model.None_VGG_BiLSTM_CTC_th"

        model_pkg = importlib.import_module(recog_network)

        print(fr'get_recognizer recog_network else : {current_path} {recog_network} ')
        print(fr'get_recognizer model_pkg else : {current_path} {model_pkg} ')
...
def get_text(character, imgH, imgW, recognizer, converter, image_list, \
    ...
    print(fr'get_text result1 : {current_path} {result1}')
    ...
    for i, zipped in enumerate(zip(coord, result1)):
        ...
        print(fr'get_text i : {current_path} {i}')
        print(fr'get_text zipped : {current_path} {zipped}')

        box, pred1 = zipped

        print(fr'get_text box : {current_path} {box}')
        print(fr'get_text pred1 : {current_path} {pred1}')
        ...
        if i in low_confident_idx:
            ...
            print(fr'get_text box : {current_path} {box}')
            print(fr'get_text pred1[0] : {current_path} {pred1[0]}')
            print(fr'get_text pred1[1] : {current_path} {pred1[1]}')
            print(fr'get_text result : {current_path} {result}')
