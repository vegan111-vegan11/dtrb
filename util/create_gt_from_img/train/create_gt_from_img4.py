...
#val_gt_file_path = r'C:\Users\TAMSystech\yjh\ipynb\deep-text-recognition-benchmark\data\ttf14\val\gt\gt.txt'
...
# gt.txt 파일 읽기
with open(train_gt_file_path_tg, 'r', encoding='utf-8') as train_gt_file_tg:
    train_gt_lines_tg = train_gt_file_tg.readlines()
...
#for root, dirs, files in os.walk(val_image_folder):
    ...
    if not dirs:
        ...
        #val_gt_file_path = fr'C:\Users\TAMSystech\yjh\ipynb\deep-text-recognition-benchmark\data\ttf14\val\gt\{}gt.txt'
        #val_gt_file_path = root.replace('img', 'gt')
        #val_gt_file_dir = root.replace('train', 'val')
        train_gt_file_dir = root.replace('img', 'gt')
        print(f'if not dirs val_gt_file_dir : {train_gt_file_dir}')

        #os.makedirs(os.path.dirname(val_gt_file_dir), exist_ok=True)

        train_gt_file_path = os.path.join(train_gt_file_dir, 'gt.txt')
        print(f'if not dirs train_gt_file_path : {train_gt_file_path}')

        os.makedirs(os.path.dirname(train_gt_file_path), exist_ok=True)
    ...
    for file in files:
        ...
        if file.lower().endswith('.jpg'):
            ...
            with open(train_gt_file_path, 'a', encoding='utf-8') as train_gt_file:
                for train_gt_line_tg in train_gt_lines_tg:
                    # 파일명 추출 (확장자 전까지)
                    ...
                    if train_gt_line_tg.split()[0] == file:
                        ...
                        # gt.txt 파일에 쓰기
                        train_gt_file.write(train_gt_line_tg)
