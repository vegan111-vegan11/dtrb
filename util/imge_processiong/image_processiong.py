...
file_path = fr'C:/Users/TAMSystech\yjh\ipynb\deep-text-recognition-benchmark/ocr_test/0115/{lan}/{lan}_전처리_with_result_updated.xlsx'
...
base_path = fr'C:\Users\TAMSystech\yjh\ipynb\deep-text-recognition-benchmark\data\{lan}\test\0115\\train\img\skew_angle(0.0)\blur(0.0)'
            ...
            for preprocessing_name, preprocessing_func in preprocessing_steps:
                ...
                for filename in os.listdir(image_dir):
                    ...
                    if filename.endswith(".jpg"):
                        ...
                        # 이미지 채널 수 확인
                        if len(image_array.shape) == 2:  # Grayscale 이미지인 경우
                            img_cv_grey = image_array
                        else:  # 컬러 이미지인 경우
                            img_cv_grey = cv2.cvtColor(image_array, cv2.COLOR_BGR2GRAY)
                        #print(img_cv_grey.shape)  # 이미지의 차원 확인
                        ...
                        # 파일로 저장
                        image_path = os.path.join(image_dir, filename)
                        image_path = os.path.join(image_dir, fr'image_processing.jpg')

                        if preprocessing_name != 'imageProcessing':
                            image_path = os.path.join(image_dir, fr'{preprocessing_name}.jpg')
                        else:
                            image_path = os.path.join(image_dir, fr'{preprocessing_name}.jpg')


                        cv2.imwrite(fr'{image_path}', preprocessed_image)
