...
data = {
    ...
    'number_of_training_images': [1052] * 7,
    'lan_of_training_images': ['vi', 'vi', 'vi', 'vi', 'vi', 'vi', 'vi']
}

df = pd.DataFrame(data)

excel_file_path = fr'C:\Users\TAMSystech\yjh\ipynb\deep-text-recognition-benchmark\ocr_test\stat\stat_{4}.xlsx'
...
for marker in used_markers:
    ...
    for batch_ratio in used_batch_ratios:
        ...
        if not batch_data.empty:
            ...
            # 결과값을 숫자로 표시 (빨간색)
            for xi, yi, zi in zip(batch_data['epoch'], batch_data['number_of_training_images'], batch_data['result']):
