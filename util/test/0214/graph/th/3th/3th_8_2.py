...
excel_file_path = fr'C:\Users\TAMSystech\yjh\ipynb\deep-text-recognition-benchmark\ocr_test\facter\{lan}\facter_{lan}2.xlsx'
...
# 중복을 제거하여 유일한 레이블 가져오기
unique_labels2 = df['Color'].unique()
...
for label in unique_labels:
    ...
    print(fr'marker : {marker}')
    print(fr'marker : {markers}')
...
#     elif label == 'white':
#         label_text = 'White'
#     sc = ax.scatter([], [], [], marker=marker, label=f'Background Color: {label_text}')  # 범례에 해당 마커를 표시합니다.
...
#     elif label == 'white':
#         label_text = 'White'
#     sc = ax.scatter([], [], [], marker='o', color=label, label=f'Background Color: {label_text}')  # 범례에 해당 마커를 표시합니다.

for label in unique_labels2:
    if label == 'black':
        label_text = 'Black'
        color = 'black'  # 라벨 색상을 녹색으로 변경
    elif label == 'white':
        label_text = 'White'
        color = 'white'  # 라벨 색상을 녹색으로 변경
    sc = ax.scatter([], [], [], marker='o', color=color, label=f'Background Color: {label_text}')  # 범례에 해당 마커를 표시합니다.
    # 텍스트 색상을 분홍색으로 설정합니다.
    # for text in ax.legend_.texts:
    #     if text.get_text() == f'Background Color: {label_text}':
    #         text.set_color('magenta')
    #sc.set_color('green')  # 텍스트 색상을 녹색으로 설정합니다.
# 색상 바 추가
# cbar = fig.colorbar(sc)
# cbar.set_label('Color', rotation=270, labelpad=20)
