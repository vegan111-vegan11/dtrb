...
def train(opt):
    ...
    print(f'트레인 처음 들어감train opt : {opt}')
    # if 'ๆ' in opt.character:
    #     # 'ๆ' 문자가 있을 경우
    #     print('트레인 처음 들어감 ๆ 있음')
    # else:
    #     # 'ๆ' 문자가 없을 경우
    #     print('트레인 처음 들어감 ๆ 없음')
    ...
    # else:
    #     # 'ๆ' 문자가 없을 경우
    #     print('트레인 처음 들어와서 Batch_Balanced_Dataset 에 opt 넘겨줄때 ๆ 있음')
    ...
    if 'CTC' in opt.Prediction:
        if opt.baiduCTC:
            ...
            print(f'train.py CTC 인 경우 opt.character 를 CTCLabelConverter 함수에 전달 opt.character : {opt.character}')
            # if 'ๆ' in opt.character:
            #     # 'ๆ' 문자가 있을 경우
            #     print('train.py CTC 인 경우 opt.character 를 CTCLabelConverter 함수에 전달 ๆ 있음')
            # else:
            #     # 'ๆ' 문자가 없을 경우
            #     print('train.py CTC 인 경우 opt.character 를 CTCLabelConverter 함수에 전달 ๆ 없음')
    ...
    if opt.saved_model != '':
        ...
        if opt.FT:
            ...
            ########################################
            # 가중치 로드시 출력개수 같은경우와 틀린경우에 따라 dict 로드, 병렬처리 순서를 틀리게 해야함.
            #######################################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_max_length', type=int, default=400, help='maximum-label-length')
...
# ğĨĩĪīĮįİıĶķĹĺĻļĽľŁłŃńŅņŇňŒœŔŕŘřŚśŞşŠšŤťŨũŪūŮůŲųŸŹźŻżŽžƏƠơƯưȘșȚțə̇ḌḍḶḷṀṁṂṃṄṅṆṇṬṭẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹ€'''
    ...
    opt.valid_data = "data_lmdb_release/ttf15/val"
    opt.train_data = "data_lmdb_release/ttf15/train"
    opt.valid_data = "data_lmdb_release/th/test/0116/val"
    ...
    if opt.num_gpu > 1:
        ...
        print(f'배치사이즈 업데이트 전 opt.num_gpu : {opt.num_gpu}')
        print(f'workers 업데이트 전 opt.workers : {opt.workers}')
        print(f'배치사이즈 업데이트 전 opt.batch_size : {opt.batch_size}')
        opt.workers = opt.workers * opt.num_gpu
        opt.batch_size = opt.batch_size * opt.num_gpu

        print(f'opt.num_gpu : {opt.num_gpu}')
        print(f'배치사이즈 업데이트 ( opt.workers * opt.num_gpu ) 후 opt.workers : {opt.workers}')
        print(f'배치사이즈 업데이트 ( opt.batch_size * opt.num_gpu ) 후 opt.batch_size : {opt.batch_size}')

        opt.batch_size = 300
        opt.batch_size = 400
        print(f'여기서 배치사이즈 업데이트 했음 300 변경후 opt.batch_size : {opt.batch_size}')
        """ previous version
